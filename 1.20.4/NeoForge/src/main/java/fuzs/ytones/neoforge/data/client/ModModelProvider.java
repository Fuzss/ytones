    ...
    protected void registerStatesAndModels() {
        ...
        Tone.forEach((tone, toneType) -> {
            ...
            texture = new ResourceLocation(texture.getNamespace(),
                    texture.getPath().replaceAll(toneType.id() + "$", String.valueOf(toneType.ordinal()))
            );
        ...
        this.getVariantBuilder(ModRegistry.FLAT_LAMP_BLOCK.value()).forAllStatesExcept(state -> {
            ...
            ModelFile modelFile = this.models()
                    .getExistingFile(new ResourceLocation(path.getNamespace(),
        ...
        this.itemModels()
                .withExistingParent(this.name(ModRegistry.FLAT_LAMP_BLOCK.value()),
                        this.extendKey(resourceLocation, ModelProvider.BLOCK_FOLDER)
                );
    }
